...
- **No one-off reminder feature — dropped after real testing.** The original plan was Slack's native `reminders.add`, on the assumption (matching Slack's own docs at the time) that a bot token could set reminders for other users even though the 2023 API change retired the user-token path for that. Live testing returned `not_allowed_token_type` for every attempt, and both Slack's community forum and outside research confirm `reminders.add` now only accepts a user token (`xoxp-...`) — a bot token is rejected outright, docs wording notwithstanding. Supporting it properly would mean a full per-user OAuth flow (its own callback endpoint, a new table to store each user's token, a "connect your account" step before first use) — a second, parallel auth model bolted onto an app whose entire design rests on one bot token and no OAuth install flow. Not worth it for one feature, especially since Slack already provides the same thing natively: anyone can right-click a message → "Remind me about this."
- **The weekly digest is what guarantees everyone with an open task gets a regular nudge.** It is Docket's own cron job, so it does not depend on any Slack API staying available. This makes it reliable by construction. It supports a `--dry-run` flag (`php bin/digest.php --dry-run`) that prints exactly what it would do — every DM, every channel post, every sweep — with no Slack calls and no storage writes, for safely previewing a run against real data.
- Each person's digest DM leads with how many tasks they completed since the last run (omitted if zero) and how many they still have open, before the existing per-channel task list.
- The weekly digest **also posts a channel-level summary**, in addition to the per-assignee DMs: how many tasks were completed, the total open count, an "⚠️ Overdue" section (any open task past due, regardless of assignee), and an "Unassigned" section — each section only appears when non-empty. This posts whenever there's something to say (a completion, an overdue task, or an unassigned task), not only when something is unassigned, so a fully-assigned channel with an overdue task still gets a heads-up.
- "Since the last check," for both the DM and the channel summary, needs no stored timestamp: the same run's sweep (see below) deletes every done row every time, so whatever's still marked done right before that sweep runs is exactly what's been completed since the previous run.
...
- **No Slack SDK dependency.** The Web API call surface (`chat.postMessage`, `chat.update`, `pins.add`, `conversations.join`, `views.open`, `views.publish`, `conversations.open`) is small enough for a thin, hand-rolled client over Guzzle.
- **Every outgoing message disables Slack's automatic link/media unfurling** (`unfurl_links: false`, `unfurl_media: false` on `chat.postMessage`/`chat.update`). Without this, a message containing one of Docket's own Slack permalinks — a row's 🔗 source link, or the list link in an assignment DM — gets a full preview attachment of the linked message auto-appended by Slack, bloating both the pinned list and the DM with content nobody asked to see.
...
- `POST /slack/interactions` — block actions (▲/▼, done, edit buttons), the "Add as task" message shortcut, and the add/edit-task modal's `view_submission`. The body is `application/x-www-form-urlencoded`, with a single `payload` field containing JSON.
...
**Scopes:** `chat:write` (to post and update the list, and to DM digests and assignments), `commands`, `pins:write`, `im:write`, `channels:join`. There is no `channels:history` or `groups:history`, because nothing here reads ambient channel messages. Message-shortcut payloads include the message text, author, channel, team domain, and message timestamp directly — enough to construct the permalink ourselves (`https://{team_domain}.slack.com/archives/{channel_id}/p{ts_without_the_dot}`), so no extra scope or API call is needed to get it.
...
- **Toggle Important**: the ⭐ block action flips `important` directly from the row, without opening the modal — a quick path alongside Edit for the one field people change often enough to want a single tap. Doesn't touch any other field or `priority`.
...
- **Weekly digest** (cron, `bin/digest.php`): DMs every assignee with open tasks a summary grouped by channel, leading with their completed-since-last-check count. Posts a channel-level summary (completed count, total open count, overdue section, unassigned section) to every channel that has something to report. Then sweeps done tasks out of every channel with any task — including a channel whose last open task was just completed, so it still gets its pinned list refreshed to show an empty list. Supports `--dry-run` for a no-op preview.
...
5. The add/edit-task modal (title, assignee, due date, Important).
...
8. ~~The `reminders.add` integration~~ — dropped after real testing showed `reminders.add` rejects bot tokens outright; see Decisions.
